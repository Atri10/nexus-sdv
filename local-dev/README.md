...
**Predictive-maintenance demo:** `make pm-demo` is the one-command path to the
fleet-health page — same stack setup, plus the `predictive-maintenance`
detector service, then opens **http://localhost:3000/pm** (see
[Predictive-maintenance demo](#9-predictive-maintenance-demo)).
...
## 9. Predictive-maintenance demo

One command runs the fleet-health showcase — stack, vehicle simulator, the
`predictive-maintenance` detector service, and the /pm dashboard:
...
`make pm-demo` is idempotent: same setup/start as `make go` (a no-op when
already configured), ensures the `vehicle-simulator` AND
`predictive-maintenance` containers are up, then opens
**http://localhost:3000/pm**. No authentication is required on the demo
pages.

The `/pm` page is the predictive-maintenance fleet view:
...
- **Validation card** — precision / recall / mean lead time from the last
  offline evaluator run (`public/pm-validation.json`, placeholder values
  until a soak is scored).
- **Fleet health matrix** — latest health score per VIN × component, click a
  cell to drill down.
- **Alert feed** — newest-first detector alerts (capped at 50).
- **Drill-down** — degradation trend (7 d), detector math from the alert's
  evidence, and predicted-vs-actual against the simulator's live ground
  truth.
...
## 10. Build model (Dockerfile.local & proto)
