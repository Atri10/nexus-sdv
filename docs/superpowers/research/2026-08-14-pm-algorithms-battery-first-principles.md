...
4. **Code-vs-spec scoring divergence**: the spec proposed a composite weighted score (60 % resting trend / 25 % cranking / 15 % SOC drift, `2026-08-05-predictive-maintenance-algorithms.md` §3.3); the code implements min-of-caps composition and has **no SOC term at all** (the processor does not even request `dynamic:battery.soc`). The weights were never transcribed into code.
