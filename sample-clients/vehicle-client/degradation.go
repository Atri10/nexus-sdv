...
// TirePressureAt returns bar at day. The tire dies in three phases, which is
// the physical story: a slow leak, then — once the carcass is underinflated —
// sidewall flexing overheats the tire and accelerates the leak, and finally
// structural collapse leaves the tire flat (~1.0 bar, "entire tyre useless").
// Healthy VINs do not leak (they must publish nothing, so their pressure
// stays at the 2.3 bar baseline).
...
// plus underinflation flex-heating: below 1.9 bar the sidewalls do extra
// work and the tire runs hot (up to +14 °C as it approaches flat). The heat
// is what makes the leak self-accelerating in real tires, and the detector
// sees it as a steeper temperature-compensated pressure decline.
func (d *DegradationConfig) TireTempAt(day float64) float64 {
	ambient := 28.0 + 8.0*math.Sin(day*2*math.Pi)
	if d.Preset == "healthy" {
		return ambient
	}
	p := d.TirePressureAt(day)
	heat := 0.0
	if p < 1.9 {
