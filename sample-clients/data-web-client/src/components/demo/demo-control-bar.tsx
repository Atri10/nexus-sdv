...
export function DemoControlBar({ vin, onVinChange, simulatorVin, running, busy, onStart, onStop }: DemoControlBarProps) {
  ...
  return (
    <Card className="flex flex-wrap items-center gap-3 p-3">
      ...
      <Button variant="secondary" size="sm" className="transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]" onClick={handleNewVehicle}>
        <Plus />
        New vehicle
      </Button>

      {running ? (
        <Button variant="destructive" size="sm" className="transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]" onClick={onStop} disabled={busy}>
          <Square />
          Stop
        </Button>
      ) : (
        <Button size="sm" className="transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]" onClick={onStart} disabled={busy}>
          <Play />
          Start
        </Button>
      )}
