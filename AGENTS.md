...
- **Predictive maintenance**: `sample-services/predictive-maintenance` polls data-api for each VIN, runs deterministic battery/brake/tire detectors, and publishes severity to `pm.{VIN}.{component}` — **tires and brakes are per-wheel/pad** (`pm.{VIN}.tires.{FL|FR|RL|RR}`, `pm.{VIN}.brake.{pad}`) with the sim emitting `TIRE_PRESSURE.{wheel}`, `TIRE_TEMP.{wheel}`, `BRAKE_WEAR.{wheel}` sensors; the web shows a worst-wheel summary + per-wheel detail. The web client streams these via SSE (`/api/pm/stream`) into the `/demo` health gauges and the `/pm` single-vehicle live console. The simulator can degrade components over time (`DEGRADATION_PRESET`, `DEGRADATION_ACCEL`) and drives demo pacing (`DEMO_SPEED` 1x/5x/20x).
